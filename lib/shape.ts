...
import { validateModelResponse } from "./output-schema";
...
export async function shape(
  ...
  const review_markdown = castToMarkdown(profile, spine, output);
  const host_json_view = castToHostJson(profile, spine, output, engine);
  const check = runCheck(profile, output, support, spine, rawText);
